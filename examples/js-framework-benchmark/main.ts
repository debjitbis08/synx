...
function createBenchmarkRow(initial: {
  ...
  const el = tr(
    {
      "data-id": R.map(row.prop, (item) => String(item.id)),
      class: rowClass,
    },
    td({ class: "col-md-1" }, R.map(row.prop, (item) => String(item.id))),
    td(
      { class: "col-md-4" },
      a(
        {
          class: "lbl",
          on: {
            click: () => initial.onSelect(R.sample(row.prop).id),
          },
        },
        label
      )
      ...
      a(
        {
          class: "remove",
          on: {
            click: () => initial.onRemove(R.sample(row.prop).id),
          },
        },
...
function createBenchmarkApp() {
  ...
  const update = () => {
    ...
    for (let i = 0; i < current.length; i += 10) {
      current[i].setLabel((label) => `${label} !!!`);
    }
  };
  ...
  const el = div(
        ...
        div(
          ...
          div(
            ...
            div(
              ...
              button(
                { id: "run", class: "btn btn-primary btn-block", type: "button", on: { click: run } },
                "Create 1,000 rows"
              )
            ),
            div(
            ...
            div(
              { class: "col-sm-6 smallpad" },
              button(
                { id: "add", class: "btn btn-primary btn-block", type: "button", on: { click: add } },
                "Append 1,000 rows"
              )
            ),
            div(
            ...
            div(
              { class: "col-sm-6 smallpad" },
              button(
                { id: "clear", class: "btn btn-primary btn-block", type: "button", on: { click: clear } },
                "Clear"
              )
            ),
            div(
    ...
    table({ class: "table table-hover table-striped test-data" }, tbody({ id: "tbody" }, rowItems)),
