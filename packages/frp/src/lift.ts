      ...
      .map((arg, index) => {
        if (arg !== null && R.isReactive(arg)) {
          ...
          return R.subscribe(reactive, (newValue) => {
            ...
            const newResult = fn(...(currentValues as UnwrapReactiveArgs<A>));
...
export function liftBinary<A, B, R>(
  ...
  (a: Reactive<UnwrapReactive<A>>, b: Reactive<UnwrapReactive<B>>): Reactive<R>;
} {
  ...
  return (a: A | Reactive<A>, b: B | Reactive<B>) => {
    if (R.isReactive(a)) {
      if (R.isReactive(b)) {
        ...
        return R.ap(
          b as Reactive<B>,
          R.map(a as Reactive<A>, (aVal: A) => (bVal: B) => fn(aVal, bVal))
        );
      } else {
